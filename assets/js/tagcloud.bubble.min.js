...
window.onload = function() {
    if(document.getElementById('bubble-tag-cloud')){
        var cl = Cloud('bubble-tag-cloud', {
            radius_x: 350,
            radius_y: 350,
            radius_z: 350,
            radius_stop: .3
        });
        jQuery(document.getElementById('bubble-tag-cloud')).animate({'opacity': 1});
    }
...
function Cloud(id, opt) {
    ...
    var cloud = document.getElementById(id),
        ...
        elements = cloud.querySelectorAll('.customer-item'),
...
//        TweenMax.to(el, 0, {});
    ...
    function recount(e) {
        ...
        x = evt.pageX - cloud_coord.left - cloud_width / 2,
        y = evt.pageY - cloud_coord.top - cloud_height / 2,
...
// function load_bubble(){var canvas=document.getElementById('bubble'),w=canvas.parentElement.offsetWidth,h=canvas.parentElement.offsetHeight,ctx=canvas.getContext('2d'),rate=25,arc=50,time,count,size=75,speed=5,lights=new Array,colors=['#f1f1f1','#f1f1f1','#f1f1f1','#f1f1f1','#f1f1f1'];canvas.setAttribute('width',w);canvas.setAttribute('height',h);function init(){time=0;count=0;for(var i=0;i<arc;i++){lights[i]={x:Math.ceil(Math.random()*w),y:Math.ceil(Math.random()*h),toX:Math.random()*5+1,toY:Math.random()*5+1,c:colors[Math.floor(Math.random()*colors.length)],size:Math.random()*size}}}
// function bubble(){ctx.clearRect(0,0,w,h);for(var i=0;i<arc;i++){var li=lights[i];ctx.beginPath();ctx.arc(li.x,li.y,li.size,0,Math.PI*2,false);ctx.fillStyle=li.c;ctx.fill();li.x=li.x+li.toX*(time*0.05);li.y=li.y+li.toY*(time*0.05);if(li.x>w){li.x=0;}
// if(li.y>h){li.y=0;}
// if(li.x<0){li.x=w;}
// if(li.y<0){li.y=h;}}
// if(time<speed){time++;}
// timerID=setTimeout(bubble,200/rate);}
// init();bubble();}
// load_bubble();